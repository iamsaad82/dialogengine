...
export class TemplateStore {
  private mapToPrismaTemplate(template: Partial<Template>): Prisma.TemplateCreateInput | Prisma.TemplateUpdateInput {
  ...
  private mapFromPrismaTemplate(template: any): Template {
    return {
      ...template,
  ...
  async findBySubdomain(subdomain: string): Promise<Template | null> {
    const template = await prisma.template.findUnique({
      where: { subdomain }
    });
    return template ? this.mapFromPrismaTemplate(template) : null;
  ...
  async create(data: Omit<Template, 'id' | 'createdAt' | 'updatedAt'>): Promise<Template> {
    ...
    const template = await prisma.template.create({
      data: prismaData as Prisma.TemplateCreateInput
    });
  ...
  async update(id: string, data: Partial<Template>): Promise<Template> {
    ...
    const template = await prisma.template.update({
      where: { id },
      data: prismaData as Prisma.TemplateUpdateInput
    });
    return this.mapFromPrismaTemplate(template);
  }
